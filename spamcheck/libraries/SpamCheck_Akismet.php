<?php
class SpamCheck_Akismet extends SpamCheck_Abstract{
	
	protected $akismet;
	
	public function __construct(SpamCheck $spamcheck)
	{
		parent::__construct($spamcheck);
		$this->akismet=new Akismet_Plus;
	}
	/**
	 * Map calls to the Akismet library
	 * 
	 */
	public function __call($method,$args)
	{
		if(method_exists($this->akismet,$method))
		{
			call_user_func_array(array($this->akismet,$method),$args);
			return $this;
		}
		throw new Kohana_Exception('spamcheck.no_such_method');
	...
	public function check(){
		$this->akismet->setCommentAuthor($this->spamcheck->get_field('author'));
		$this->akismet->setCommentAuthorEmail($this->spamcheck->get_field('author_email'));
		$this->akismet->setCommentAuthorURL($this->spamcheck->get_field('author_url'));
		$this->akismet->setCommentContent($this->spamcheck->get_field('content'));

		return $this->akismet->isCommentSpam();
	}
}

class Akismet_Plus extends Akismet{
	/**
	 *	@param	string	$blogURL			The URL of your blog.
	 *	@param	string	$wordPressAPIKey	WordPress API key.
	 */
	public function __construct($blogURL=null, $wordPressAPIKey=null) {

		$this->setBlogURL($blogURL);
		$this->setAPIKey($wordPressAPIKey);
...
class Akismet
	...
	/**
	 *	Set the blog url for the api and the comment
	 *
	...
	/**
	 *	Set API key for Akismet
	 *
	 *	@param string $wordPressAPIKey
	 */	
	public function setAPIKey($wordPressAPIKey)
	{
